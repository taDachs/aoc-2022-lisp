...
;; round, the players each simultaneously choose one of Rock, Paper, or Scissors using a
...
;; shape you selected (1 for Rock, 2 for Paper, and 3 for Scissors) plus the score for the outcome
...
(defun process-file-part-1 (c)
  (mapcar
    (lambda (x)
      (let ((you (cond
                   ((equal (subseq x 0 1) "A") 'rock)
                   ((equal (subseq x 0 1) "B") 'paper)
                   ((equal (subseq x 0 1) "C") 'scissor)))
            (me (cond
                  ((equal (subseq x 2 3) "X") 'rock)
                  ((equal (subseq x 2 3) "Y") 'paper)
                  ((equal (subseq x 2 3) "Z") 'scissor))))
        (list you me)))
    c))

(defun process-file-part-2 (c)
  (mapcar
    (lambda (x)
      (let ((you (cond
                   ((equal (subseq x 0 1) "A") 'rock)
                   ...
                   ((equal (subseq x 0 1) "C") 'scissor)))
            (me (cond
                  ((equal (subseq x 2 3) "X") 'lose)
                  ((equal (subseq x 2 3) "Y") 'draw)
                  ((equal (subseq x 2 3) "Z") 'win))))
        (list you (get-required-move you me))))
    c))

(defun get-losing-move (you)
  (case you
    ('rock 'scissor)
    ('paper 'rock)
    ('scissor 'paper)))

(defun get-winning-move (you)
  (case you
    ('rock 'paper)
    ('paper 'scissor)
    ('scissor 'rock)))

(defun points-for-action (x)
  (case x
    ('rock 1)
    ('paper 2)
    ('scissor 3)))
...
(defun get-required-move (you needed)
  (case needed
    ('lose (get-losing-move you))
    ('draw you)
    ('win (get-winning-move you))))

(defun is-draw (you me)
  (equal me you))

(defun did-i-win (you me)
  (case me
    ('rock (equal you 'scissor))
    ('paper (equal you 'rock))
    ('scissor (equal you 'paper))))

(defun points-for-result (you me)
  (cond
    ((is-draw me you) 3)
    ((did-i-win me you) 6)
    (t 0)))
...
(defun points-per-round (you me)
  (+ (points-for-result me you) (points-for-action me)))

(defun get-exp-result (l)
  (apply '+ (mapcar (lambda (x) (apply #'points-per-round x)) l)))

(format t "# PART 1 # Expected Result: ~d~%"
        (get-exp-result (process-file-part-1 (get-file "./input.txt"))))

(format t "# PART 2 # Expected Result: ~d~%"
        (get-exp-result (process-file-part-2 (get-file "./input.txt"))))
